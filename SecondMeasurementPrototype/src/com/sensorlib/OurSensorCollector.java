 ...
 */
public class OurSensorCollector implements SensorEventListener, Closeable,
		Listener, LocationListener, NmeaListener {

	// Fudge factor to allow us to compile under Froyo. We copy the constant
	// from
	// Gingerbread source.
	private static final int SENSOR_LINEAR_ACCELERATION = 10;

	// Some constants
	private static final String TAG = "OurSensorCollector";

	// The log that we'll be writing to.
	private final SensorLog log;

	// The context wrapper that we'll use for accessing system services,
	// receiving
	 ...
	 * @param point
	 */
	public OurSensorCollector(Context context, File sensorLogPath,
	 ...
	 */
	public OurSensorCollector(Context context, SensorLog log) {
	...
	public synchronized void start() {
		...
		Location loc = p != null ? locationManager.getLastKnownLocation(p)
				: null;
		if (loc != null) {
	 ...
	 * 
	 * Note that pausing the sensors will create a discontinuity in (for
	 * example) the gyro readings, which will make it impossible to integrate
	 * their values across this point. To make the surveys more useful, I'd
	 * expect that you want to instruct your users to do something to make
	 * interpretation of these gaps more consistent. For example, you could
	 * instruct a user to face north whenever they pause or resume a survey.
	 */
	...
	/**
	 * Resumes the collectors if they were previously paused. Automatically
	 * writes a note to the log that a resume has occurred.
	 */
