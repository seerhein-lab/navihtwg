...
public class LocationMeasuringPoint extends MeasuringPoint {
	...
	public LocationMeasuringPoint(MeasuringPosition location,
			...
			double longitude, double altitude, float accuracy, String device) {
	...
	public String getWritableData() {
		String writableData = getLocation().getID() + Constants.SEPERATOR
				...
				+ device + Constants.SEPERATOR + Double.toString(getLatitude())
				+ Constants.SEPERATOR + Double.toString(getLongitude())
				+ Constants.SEPERATOR + Double.toString(getAltitude())
				+ Constants.SEPERATOR + Float.toString(getAccuracy())
				+ Constants.SEPERATOR;

		writableData = writableData.replace(".", ",");
		writableData += linkToMaps + Double.toString(getLatitude()) + ","
				+ Double.toString(getLongitude()) + Constants.NEWLINE;
